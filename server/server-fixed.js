...
function getCryptoData() {
    return new Promise((resolve, reject) => {
        ...
        console.log('[Crypto] 获取CoinMarketCap数据...');
            ...
            res.on('end', () => {
                try {
                    ...
                    if (result.status && result.status.error_code === 0) {
                        ...
                        console.log('[Crypto] 数据更新成功，共', cryptoData.length, '条');
                        resolve(cryptoCache.data);
                    } else {
                        console.error('[Crypto] API错误:', result.status.error_message);
                        reject(new Error('CoinMarketCap API错误: ' + (result.status.error_message || '未知错误')));
                    }
                } catch (err) {
                    console.error('[Crypto] 解析失败:', err.message);
        ...
        req.on('error', (err) => {
            console.error('[Crypto] 请求失败:', err.message);
            reject(new Error('请求失败: ' + err.message));
        });
        
        req.on('timeout', () => {
            req.abort();
            console.error('[Crypto] 请求超时');
...
function handleAsrProxy(req, res, config) {
    ...
    req.on('end', () => {
        ...
        console.log('[ASR] 音频:', buffer.length, '字节');
        ...
        const apiReq = https.request(options, (apiRes) => {
            ...
            apiRes.on('end', () => {
                console.log('[ASR] 结果:', responseData);
                res.writeHead(200, {
