<template>
	<view class="container">
		...
		<view class="crypto-section">
			<crypto-carousel :cryptoData="cryptoList" />
		</view>
		...
		<scroll-view class="chat-section" scroll-y :scroll-top="scrollTop" @scrolltoupper="onScrollToUpper">
			...
			<message-bubble v-for="(msg, index) in messages" :key="index" :message="msg" />
		</scroll-view>
	...
	methods: {
		...
		async startRecord() {
			...
			try {
				...
				const selectedType = candidateTypes.find(t => typeof MediaRecorder !== 'undefined' && MediaRecorder.isTypeSupported && MediaRecorder.isTypeSupported(t))
				console.log('[H5] MediaRecorder类型:', selectedType || 'default')
				...
				this.mediaRecorder.onstop = async () => {
					...
					const audioType = selectedType || this.mediaRecorder.mimeType || 'audio/webm'
			...
			if (!this.recorderManager) {
				console.warn('[Main] recorderManager未初始化，尝试重新获取')
				try {
					this.recorderManager = uni.getRecorderManager()
					this.setupRecorderEvents()
				} catch (error) {
					console.error('[Main] 获取录音管理器失败:', error)
					uni.showToast({
						title: '录音功能不可用',
						icon: 'none'
					})
		...
		stopRecord() {
			...
			if (this.recorderManager) {
				...
				console.warn('[Main] recorderManager为空，无法停止录音')
			}
			// #endif
		},
